...
def create_reply(discussion_id):
	...
	try:
		...
		# Validate data
		validated_data = discussion_reply_schema.load(data)
  
		if not isinstance(validated_data, dict):
			return jsonify({'error': 'Invalid payload format'}), 400

		reply = discussion_reply_controller.create(validated_data)
		if reply:
...
def update_reply(reply_id):
	...
	try:
		...
		# Validate data (partial update)
		validated_data = discussion_reply_schema.load(data, partial=True)
  
		if not isinstance(validated_data, dict):
			return jsonify({'error': 'Invalid payload format'}), 400
		
		reply = discussion_reply_controller.update(reply_id, validated_data)
		if reply:
